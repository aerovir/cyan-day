...
> **Про сеть:** контейнер работает в **host-сети** (`network_mode: host`). На машинах
> за **серым IP провайдера** (CGNAT) bridge-NAT Docker может не пропускать исходящий
> трафик контейнера (все запросы дают ConnectTimeout). Host-сеть решает это: контейнер
> ходит в интернет как хост. Бот делает только исходящие запросы (VK, Mistral, calend.ru),
> транслировать входящие не требуется.
